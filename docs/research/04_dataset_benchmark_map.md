...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TempGlitch | [S-003](17_source_verification_log.md) | Gameplay videos | Public HF artifact exposes binary per-video labels plus category folders; no finer public span file verified | Temporal behavior is the task focus, but the public artifact currently exposes binary per-video labels only | verified-online=yes; public-download=yes; license=MIT; downloaded-locally=no stable subset before Phase 2; converted-locally=no; evaluated-locally=no | Primary Phase 2 smoke benchmark and main paper-facing binary clip-level benchmark candidate | Public artifact has only one `train` split, an empty README, and no verified finer temporal-span annotations | Public HF dataset page verified on 2026-06-08 with 1,500 MP4 files across five balanced `Buggy` / `Normal` category pairs. See [19_benchmark_access_resolution.md](19_benchmark_access_resolution.md). |
