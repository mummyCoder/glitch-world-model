...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TempGlitch | [S-003](17_source_verification_log.md) | Gameplay videos | Public HF artifact exposes binary per-video labels plus category folders; local smoke conversion maps buggy videos to full-video positive intervals | Temporal behavior is the task focus, but the public artifact currently exposes binary per-video labels only | verified-online=yes; public-download=yes; license=MIT; downloaded-locally=yes (tiny smoke subset); converted-locally=yes (full-video interval mapping); evaluated-locally=yes (Blinking smoke subset) | Primary public-benchmark smoke path now; expand to a larger repo-defined split before any paper-facing benchmark claim | Public artifact has only one `train` split, an empty README, and no verified finer temporal-span annotations; current local result uses only two videos from one category | Public HF dataset page verified on 2026-06-08 with 1,500 MP4 files across five balanced `Buggy` / `Normal` category pairs. On 2026-06-08 this repo produced `manifest.csv`, labels CSV, `scores.csv`, and `metrics.json` for a two-video `Blinking` smoke subset. See [19_benchmark_access_resolution.md](19_benchmark_access_resolution.md) and [20_phase2_dataset_pipeline_report.md](20_phase2_dataset_pipeline_report.md). |
